...
| AI Provider | Abstraction via `lib/ai/provider.ts` (OpenAI + Ollama registered) |
| Local AI | Ollama (models tested: qwen2.5-coder:3b, qwen2.5:3b, qwen2.5:7b, llama3.2:3b) |
| Cloud AI | OpenAI (gpt-4o-mini) — **currently insufficient_quota** |
...
- **RAG is NOT a gate** for these. When RAG returns no context, the orchestrator proceeds to the LLM with a "general conversation" prompt (verified in code).
- **Status:** 🟡 IMPLEMENTED_NOT_VERIFIED (architecture in place; quality blocked by model).
...
**Current provider:** `AI_PROVIDER=ollama`, `OLLAMA_MODEL=qwen2.5-coder:3b` (set in `.env.local`). This is NOT suitable for production conversational Arabic.
...
### Critical
- **AI conversational quality:** No installed model produces acceptable Levantine-Arabic responses at usable speed. OpenAI quota is exhausted. This blocks the 22 acceptance tests and the "real receptionist" experience.
...
The 22 conversational acceptance scenarios require a production-quality Arabic conversational model. The current local models time out or produce poor Arabic. Per the instruction "if not tested, do not write PASS", all 22 are marked **NOT RUN / BLOCKED**.
...
```text
Project Status:        ACTIVE DEVELOPMENT — backend + dashboard + booking verified; conversational AI blocked by model
...
AI:                    🚫 BLOCKED (no suitable Arabic model; OpenAI quota exhausted)
Patient Chat:          🟡 IMPLEMENTED_NOT_VERIFIED (architecture + UI gating done; conversational quality blocked)
...
Browser Acceptance:    🚫 NOT RUN / BLOCKED (22 scenarios require a working conversational model)
Production Readiness:  🚫 NOT READY — requires a production-quality Arabic AI provider/model
```
